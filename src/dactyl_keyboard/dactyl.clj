...
(def usb-holder-size [8.4 10.0 2.6])
...
(def usb-hole-position (let [col0 (key-position 0 0 (map + (wall-locate2 0 1) [0 (/ mount-height 2) 0]))
                             col1 (key-position 1 0 (map + (wall-locate2 0 1) [0 (/ mount-height 2) 0]))
                             x (/ (+ (first col0) (first col1)) 2)
                            ]
                            (cons x (rest col0))
                            ))
(def usb-hole-height 10)
; length thickness height
(def usb-receptacle-size [8.8 2 3.0])
(def usb-c-hole-cube-size (cons (- (first usb-receptacle-size) (last usb-receptacle-size)) (rest usb-receptacle-size)))
(def usb-hole
    (->> (union 
            (rotate (deg2rad 90) [1 0 0] (translate [(- 0 (/ (first usb-c-hole-cube-size) 2)) 0 0] (cylinder [(/ (last usb-c-hole-cube-size) 2) (/ (last usb-c-hole-cube-size) 2)] (second usb-receptacle-size))))
            (rotate (deg2rad 90) [1 0 0] (translate [(/ (first usb-c-hole-cube-size) 2) 0 0] (cylinder [(/ (last usb-c-hole-cube-size) 2) (/ (last usb-c-hole-cube-size) 2)] (second usb-receptacle-size))))
            (apply cube usb-c-hole-cube-size)
         )
         (translate [(first usb-hole-position) (- (second usb-hole-position) (/ (- wall-thickness (second usb-receptacle-size)) 2)) usb-hole-height ])))
         ; x y height
         ; (translate [-45 20 10])))
         ; line 27853
(def usb-outter-size [(* (first usb-receptacle-size) 2) (- (* wall-thickness 2) (second usb-receptacle-size)) (* (last usb-receptacle-size) 2)])
(def usb-outter-cube-size (cons (- (first usb-outter-size) (last usb-outter-size)) (rest usb-outter-size)))
(def usb-outter-hole
    (->> (union 
            (rotate (deg2rad 90) [1 0 0] (translate [(- 0 (/ (first usb-outter-cube-size) 2)) 0 0] (cylinder [(/ (last usb-outter-cube-size) 2) (/ (last usb-outter-cube-size) 2)] (second usb-outter-size))))
            (rotate (deg2rad 90) [1 0 0] (translate [(/ (first usb-outter-cube-size) 2) 0 0] (cylinder [(/ (last usb-outter-cube-size) 2) (/ (last usb-outter-cube-size) 2)] (second usb-outter-size))))
            (apply cube usb-outter-cube-size)
         )
         (translate [(first usb-hole-position) (+ (second usb-hole-position) wall-thickness) usb-hole-height ])))
